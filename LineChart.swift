...
import QuartzCore
        ...
        func pan(recognizer: UIPanGestureRecognizer) {
            if popoverController != nil {
                if recognizer.state == UIGestureRecognizerState.Began {
                    ...
                    var touchLocation = lineChart.convertPoint(recognizer.locationInView(self), toLayer: lineChart.lines.first)
                    if let point = lineChart.closestPointTo(touchLocation) {
                        if popoverPoint == nil || popoverPoint! != point {
                            ...
                            let rect = CGRect(origin: lineChart.convertPoint(point.position, fromLayer: lineChart.lines.first), size: CGSize(width: 1.0, height: 1.0))
    ...
    class SimpleLineChartTooltop : UIViewController, LineChartTooltip {
        ...
        var point: LineChart.Point? {
            didSet {
                valueLabel.text = point == nil ? "" : point!.value.shortFormatted
                lineLabel.text = point == nil || point!.line == nil ? "" : point!.line!.label
            }
        }
    ...
    class LineChartView: NSView {
        ...
        override func mouseMoved(theEvent: NSEvent) {
            ...
            var mouseLocation = lineChart.convertPoint(theEvent.locationInWindow, toLayer: lineChart.lines.first)
        ...
        override var representedObject: AnyObject? {
            didSet {
                if let point = representedObject as? LineChart.Point {
                    ...
                    lineLabel.stringValue = point.line!.label
                }
            }
        }
    }
...
class LineChart: CALayer {
    ...
    var lines: [Line] = [Line]() {
        didSet {
            for line in oldValue {
                line.removeFromSuperlayer()
                line.legendElement.removeFromSuperlayer()
            }
            for line in lines {
                if line.xAxis == nil {
                    line.xAxis = xAxis
                ...
                if line.yAxis == nil {
                    line.yAxis = self.yAxes.first!
                }
                if !contains(yAxes, line.yAxis) {
                    yAxes += [line.yAxis]
                }
                if line.strokeColor == nil {
                    let colorIndex = (lines.count - 1) % 5
                    line.color = defaultColors[colorIndex]
                }
                line.delegate = self
                addSublayer(line)
                legend.addSublayer(line.legendElement)
            }
            updateXAxis()
            updateLegend()
        }
    ...
    var title: CATextLayer = CATextLayer()
    var legend = CALayer()
    ...
    func updateXAxis() {
        ...
        let maxXTickValue = CGFloat( lines.reduce(0, {$0 > $1.data.count ? $0 : $1.data.count }) - 1 )
    ...
    func updateLegend() {
        ...
        for line in lines {
            ...
            line.legendElement.frame = CGRect(origin: lastLegendElementOrigin, size:line.legendElement.frame.size)
            lastLegendElementOrigin = CGPoint(x: lastLegendElementOrigin.x + line.legendElement.frame.width + 20.0, y: 0.0)
        }
    ...
    override func layoutSublayers() {
        ...
        for yAxis in yAxes {
            let linesForAxis = lines.filter { $0.yAxis == yAxis }
            
            // figure out how many Y-Axis tick marks look best and at what interval
            let minYValue = linesForAxis.reduce(CGFloat.max, { min($0, minElement($1.data)) })
            let maxYValue = linesForAxis.reduce(CGFloat.min, { max($0, maxElement($1.data)) })
        ...
        // 7) Refresh the individual lines
        for line in lines {
            line.layoutSublayers()
        }
    }
...
extension LineChart {
    ...
    class Axis : CALayer {
        ...
        override func layoutSublayers() {
            //println("Axis: layoutSublayers")
            if size == nil {
                return
            }
            axisLine.path = CGPath.Line(CGPoint(x: 0, y: 0), end: CGPoint(x: 0, y: size.height))
        ...
        func showGrid(width: CGFloat) {
            for tick in ticks {
                if tick.major {
                    ...
                    tick.lineLayer.path = CGPath.Line(CGPoint(x: xStart, y: 0), end: CGPoint(x: xEnd, y: 0))
                }
            }
        }
    }
    class Tick : CALayer {
        ...
        init(value: CGFloat, alignment: Alignment, major: Bool) {
            ...
            lineLayer.path = CGPath.Line(CGPoint(x: xStart, y: 0), end: CGPoint(x: xEnd, y: 0))
            ...
            /*labelLayer.string = value.shortFormatted
            labelLayer.sizeToFit()
            
            if alignment == .Left {
                labelLayer.position = CGPoint(x: -12.0 - (labelLayer.bounds.width / 2.0), y: 0.0)
                labelLayer.alignmentMode = kCAAlignmentRight
            } else {
                labelLayer.position = CGPoint(x: 12.0 + (labelLayer.bounds.width / 2.0), y: 0.0)
                labelLayer.alignmentMode = kCAAlignmentLeft
                
                println("Setting labelLayer.position = 12 + (\(labelLayer.bounds.width) / 2.0)")
            }*/
    ...
    class Line : CAShapeLayer {
        ...
        var bezierCurveTension: CGFloat = 0.3
        
        //var pointRadius: CGFloat = 10.0
        //var pointFillColor: CGColorRef!
        //var pointStrokeColor: CGColorRef!
        ...
        override func layoutSublayers() {
            ...
            path = bezierCurve ? CGPath.SplineCurve(positions, tension: bezierCurveTension) : CGPath.Polyline(positions)
        }
    }
    ...
    func closestPointTo(refPoint: CGPoint) -> Point? {
        ...
        for line in lines {
            for point in line.points {
                //let dist = sqrt(pow(refPoint.x - point.position.x, 2) + pow(refPoint.y - point.position.y, 2));
...
extension CGPath {
    class func Line(start: CGPoint, end: CGPoint) -> CGPath {
